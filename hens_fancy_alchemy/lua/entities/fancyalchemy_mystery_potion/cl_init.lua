...
CreateMaterial("AAnimatedMysteryColour", "VertexLitGeneric", 
		{ 
		["$basetexture"] = "fancyalchemy/potion_white",
		}
	)
...
function ENT:Draw()
	self:DrawModel()
	--self:SetSubMaterial(1,"!AAnimatedMysteryColour")
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 550 then
		...
		cam.Start3D2D(self:GetPos() + offset + Vector(0,0,10 + (math.sin(self.time*1)*2)), a , 0.11)
			...
			draw.SimpleTextOutlined("Mystery Potion","FALCH_Stacks", 0,0,Color(255,255,255,255*alpha) , 1, 1, 2, Color(0, 0, 0, 55*alpha))
		cam.End3D2D()
	end
