...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 400 then
		...
		cam.Start3D2D(Pos+PosOffset+Ang:Up()*60+Ang:Right()*-10, Ang, 0.15)
			cam.IgnoreZ(true)
			draw.SimpleTextOutlined( "Level "..self:GetNWInt("level"), "HUDNumber5", -TextWidth*0.5 + 5, 0, GrabLevelColor(self:GetNWInt("level")), 0, 0, 3, Color(84,0,94) )
			cam.IgnoreZ(false)
		cam.End3D2D()
		
		cam.Start3D2D(Pos+Ang:Up()*60+Ang:Right()*-10, Ang, 0.13)
			cam.IgnoreZ(true)
