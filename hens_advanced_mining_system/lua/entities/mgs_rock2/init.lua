...
local ExtraSpawnTime = 15
local JChance = 0

function ENT:Initialize()
	self:SetModel(table.Random(MGS2_ROCK_MODELS))
	self:SetMaterial("models/props/CS_militia/militiarockb");
	...
	self:SetNWInt("level", math.random(MGS2_MIN_LEVEL, MGS2_MAX_LEVEL))
end
...
function ENT:Think()
	if (!self.Replace) and (self:GetNWInt("health") <= 0)  then
		...
		for i=1, math.Round(ores) do
			...
			--Set the correct tier for the ore
			ore:SetNWInt("tier", 2);
			ore:SetNWInt("ore_owner", self:GetNWInt("rockkiller"))
			--Make sure we are using the appropriate level ranges
			ore:SetNWInt("flucVal", (self:GetNWInt("level") - MGS2_MIN_LEVEL) / (MGS2_MAX_LEVEL - MGS2_MIN_LEVEL))
				...
				if(plr:GetNWBool("perk_candy")) then
					...
					if(roll > 0.5) then
						for i=1, 2 do
							...
							cand:SetNWInt("he_candy_amnt", math.random(3, 6))
	...
	if(dmg:GetInflictor():IsPlayer()) then
		if table.HasValue(MGS_MINING_TOOLS, dmg:GetInflictor():GetClass()) or table.HasValue(MGS_MINING_TOOLS, dmg:GetAttacker():GetActiveWeapon():GetClass()) then
			...
			if(plr:GetNWInt("level") >= 15) then
				...
				--Starting at steel, you do double damage to lower tier rocks
				if(plr:GetNWInt("level") >= 20) then
					self:SetNWInt("health", self:GetNWInt("health")-1)
				end
