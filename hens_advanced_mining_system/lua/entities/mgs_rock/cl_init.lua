...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 400 then
		...
		cam.Start3D2D(Pos+PosOffset+Ang:Up()*10+Ang:Right()*-60, Ang, 0.15)
			draw.SimpleTextOutlined( "Level "..self:GetNWInt("level"), "HUDNumber5", -TextWidth*0.5 + 5, 0, GrabLevelColor(self:GetNWInt("level")), 0, 0, 2, Color(0,0,0) )
		cam.End3D2D()
		
		cam.Start3D2D(Pos+Ang:Up()*10+Ang:Right()*-60, Ang, 0.13)
