...
function ENT:Initialize()
	self:SetModel(table.Random(MGS1_ROCK_MODELS))
	...
	self:SetNWInt("level", math.random(MGS1_MIN_LEVEL, MGS1_MAX_LEVEL))
end
...
function ENT:Think()
	if (!self.Replace) and (self:GetNWInt("health") <= 0)  then
		...
		for i=1, math.Round(ores) do
			...
			ore:SetNWInt("flucVal", (self:GetNWInt("level") - MGS1_MIN_LEVEL) / (MGS1_MAX_LEVEL - MGS1_MIN_LEVEL))
				...
				if(plr:GetNWBool("perk_candy")) then
					...
					if(roll > 0.5) then
						for i=1, 2 do
							...
							cand:SetNWInt("he_candy_amnt", math.random(2, 4))
