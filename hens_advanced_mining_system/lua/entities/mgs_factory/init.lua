	...
	if self.CanUse then
		if (hitEnt:GetClass() == "mgs_cart") and (hitEnt:GetNWInt("ore") > 0) and (!hitEnt:IsPlayerHolding()) then
			...
			if (MGS_NEW_TIME) then	
				...
				if(hitEnt:Getowning_ent():GetNWBool("perk_chronofactories"))then
					self:SetNWInt("fulltime", math.Round(hitEnt:GetNWInt("fulltime")*0.2))
				end
	...
	if !self.CanUse then
		...
		if self.Time <= CurTime() then
			...
			if (cart != NULL) then
				...
				cart:SetNWInt("ore", 0)
				cart:SetParent()
				cart:SetPos(self:GetPos() + selfAng:Forward()*50 + selfAng:Right()*50)
				cart.CanUse = true
				...
				if (owner ~= NULL and owner:GetNWBool("perk_magicmoney") == true) then 
					if (MGS_GM_VERSION <= 2.4) then
						owner:AddMoney(amount or 0)
						DarkRP.notify(owner, 0, 4, "[MINING] You've gained $"..string.Comma(amount).." from your cart!")
					else
						owner:addMoney(amount or 0)
						DarkRP.notify(owner, 0, 4, "[MINING] You've gained $"..string.Comma(amount).." from your cart!")
					end
