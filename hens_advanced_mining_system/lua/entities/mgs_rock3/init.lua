...
function ENT:Initialize()
	self:SetModel(table.Random(MGS3_ROCK_MODELS))
	self:SetMaterial("models/props_lab/door_klab01");
	...
	self:SetNWInt("level", math.random(MGS3_MIN_LEVEL, MGS3_MAX_LEVEL))
end
...
function ENT:Think()
	if (!self.Replace) and (self:GetNWInt("health") <= 0)  then
		...
		for i=1, math.Round(ores) do
			...
			ore:SetNWInt("flucVal", (self:GetNWInt("level") - MGS3_MIN_LEVEL) / (MGS3_MAX_LEVEL - MGS3_MIN_LEVEL))
				...
				if(plr:GetNWBool("perk_candy")) then
					...
					if(roll > 0.5) then
						for i=1, 2 do
							...
							cand:SetNWInt("he_candy_amnt", math.random(5, 10))
							cand:SetPos(self:WorldSpaceCenter() + Vector(math.Rand(1,20), math.Rand(1,20),20))
							cand:Spawn()
