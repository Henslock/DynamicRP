...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		if(self.Owner:GetNWBool("hpwr_pvptoggle")) then
			draw.SimpleTextOutlined("PVP", "HPW_font2", x +5, y  - 120, Color(235,35,35), 0, 0, 2, Color(105,25,25))
		else
			draw.SimpleTextOutlined("PVP", "HPW_font2", x +5, y  - 120, Color(255,255,255, 15), 0, 0, 2, Color(0,0,0, 15))
		end
	...
	function SWEP:HPWDecreaseAccuracy(amount)
		...
		if not HpwRewrite.CVars.NoAccuracy:GetBool() then
			...
			self.HpwRewrite.CooldownAccuracy = CurTime() + 3.5
		end
	end
	...
	function SWEP:AttackSpell(curspell)
		if ((self:GetHPWAccuracyValue() + curspell.AccuracyDecreaseVal) >= 1) == true then
...
function SWEP:Deploy(anim)
	...
	if(math.random(1, 5) == 1) then
		self.Owner:ChatPrint("[TIP] You can toggle wizard PvP combat with the chat command !pvp")
		return
	end
	...
	if SERVER then
		if self.HpwRewrite.Accuracy and self.HpwRewrite.CooldownAccuracy then
			if CurTime() > self.HpwRewrite.CooldownAccuracy then
				self.HpwRewrite.Accuracy = math.Approach(self.HpwRewrite.Accuracy, 0, FrameTime() * 0.10)
			end
...
hook.Add("PlayerSay", "hpwr_togglePvP", function(ply, txt, team, isdead)
	if(string.lower(txt) == "!pvp") then
		local pvpState = !(ply:GetNWBool("hpwr_pvptoggle")) // Set the var as the opposite of the current pvp state
		
		if(pvpState) then
			ply:SetNWBool("hpwr_pvptoggle", pvpState)
			ply:SetNWFloat("hpwr_pvptogglecd", CurTime()+HPWR_PVPCD)
			ply:ChatPrint("[WARNING] You have enabled Wizard PvP. Dueling, damaging, and incapacitating spells will now be able to affect you!")
			sound.Play("hpwrewrite/pvpdrums.mp3", ply:GetPos(), 80, math.random(100, 110))
		else
			local cd = ply:GetNWFloat("hpwr_pvptogglecd") - CurTime()
			if(cd <= 0) then
				ply:SetNWBool("hpwr_pvptoggle", pvpState)
				ply:ChatPrint("You have disabled Wizard PvP. You will no longer be affected by malicious spells!")
			else
				ply:ChatPrint("[WARNING] Please wait " .. string.ToMinutesSeconds(cd) .. " before unflagging!")
			end
		end
...
local flagMat = Material( "materials/vgui/pvp_shield.png","smooth"  )

hook.Add("PostDrawOpaqueRenderables", "DrawWizardPvPFlag", function()
	...
	for i=1, #plyall do
		...
		if(ply:GetNWBool("hpwr_pvptoggle")) then
			local a = Angle(0,0,0)
			a:RotateAroundAxis(Vector(1,0,0),90)
			a.y = LocalPlayer():GetAngles().y - 90
			
			cam.Start3D2D(ply:GetPos() + Vector(0,0,85), a, 0.25)
				surface.SetDrawColor(255,255,255,255)
				surface.SetMaterial(flagMat)
				surface.DrawTexturedRect(-37.5, -37.5, 75, 75)
			cam.End3D2D()
		end
