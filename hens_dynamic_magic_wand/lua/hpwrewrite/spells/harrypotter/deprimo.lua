	...
	for k, v in pairs(ents.FindInSphere(tr.HitPos, radius)) do
		...
		if v:GetClass() == "prop_physics" then
			...
			if phys:IsValid() then 
				...
				--phys:Wake()
				phys:AddVelocity(Vector(math.random(-200,200), math.random(-200,200), math.random(350,500)))
			end
		end
		
		if(v:IsPlayer()) then v:SetVelocity(Vector(math.random(-200,200), math.random(-200,200), math.random(150,300))) end
	end
