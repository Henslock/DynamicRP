...
function Spell:AfterCollide(spell, data)
	...
	if IsValid(data.HitEntity) and data.HitEntity:IsPlayer() then
		...
		if(data.HitEntity:GetNWBool("hpwr_pvptoggle") == false) then return end
		
		if (data.HitEntity:GetNWBool("hpwr_pvptoggle") and self.Owner:GetNWBool("hpwr_pvptoggle") == false) then
	...
	if mat == MAT_CONCRETE or mat == MAT_TILE then
		...
		for i = 1, 4 do sound.Play("physics/concrete/concrete_break" .. math.random(2, 3) .. ".wav", pos, 80, 150, 0.5) end
	...
	elseif mat == MAT_WOOD then
		...
		for i = 1, 6 do sound.Play("physics/wood/wood_crate_break" .. math.random(1, 5) .. ".wav", pos, 58, 150, 0.5) end
	...
	elseif mat == MAT_METAL then
		...
		for i = 1, 3 do sound.Play("physics/metal/metal_box_break" .. math.random(1, 2) .. ".wav", pos, 72, 150, 0.1) end
	...
	elseif mat == MAT_GLASS then
		...
		for i = 1, 4 do sound.Play("physics/glass/glass_sheet_break" .. math.random(1, 3) .. ".wav", pos, 68, 150, 0.5) end
	...
	if !(self.Owner == ent:CPPIGetOwner()) then ent = NULL end
