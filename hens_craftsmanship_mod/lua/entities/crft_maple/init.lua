...
local LOG_TYPE = "mat_maplewood"

function ENT:Initialize()
	self:SetModel(TREE_MODELS.MAPLE)
	self:PhysicsInit(SOLID_VPHYSICS)
	self:SetMoveType(MOVETYPE_VPHYSICS)
	self:SetSolid(SOLID_VPHYSICS)
	self:DrawShadow(false)
	if(CRFT_HOLIDAYTREES) then
	self:SetSubMaterial(1, "crafting/mapleleafMat_frosted")
	self:SetSubMaterial(0, "crafting/mapletreeMat_frosted")
	else
	self:SetSubMaterial(1, table.Random({"crafting/mapleleafMat", "crafting/mapleleafMat_green", "crafting/mapleleafMat_yellow", "crafting/mapleleafMat_orange"}))
	end
...
function ENT:Think()
	local ign_timername = "CRFT_Maple_Ign_".. self:EntIndex()
	...
	if (!self.Replace) and (self:GetIgniteDamage()>=10) then
		if(!timer.Exists(ign_timername)) then
			self:SetSubMaterial(0, "crafting/mapleBurntMat")
			self:SetSubMaterial(1, "crafting/mapleleafMat_burnt")
