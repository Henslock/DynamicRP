...
local LOG_TYPE = "mat_birchwood"

function ENT:Initialize()
	self:SetModel(TREE_MODELS.BIRCH)
	self:PhysicsInit(SOLID_VPHYSICS)
	self:SetMoveType(MOVETYPE_VPHYSICS)
	self:SetSolid(SOLID_VPHYSICS)
	self:DrawShadow(false)
	if(CRFT_HOLIDAYTREES) then
	self:SetSubMaterial(0, "crafting/LeafTex_bare")
	else
	self:SetSubMaterial(0, table.Random({"crafting/LeafTex_orange", "crafting/LeafTex", "crafting/LeafTex_red",  "crafting/LeafTex_blue", "crafting/LeafTex_darkgreen"}))
	end
...
function ENT:Think()
	local ign_timername = "CRFT_Birch_Ign_".. self:EntIndex()
	...
	if (!self.Replace) and (self:GetIgniteDamage()>=10) then
		if(!timer.Exists(ign_timername)) then
			self:SetSubMaterial(1, "crafting/birchBurntMat")
